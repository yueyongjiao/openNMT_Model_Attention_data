...
def main():
    ...
    for batch in data_iter:
        ...
        for trans in translations:
            ...
            f.write('\n')
            f.write('='*10+'\n')
            f.write('num:'+str(fcount)+'\n')
            fcount += 1
            f.write('src:'+' '.join(trans.src_raw))
            f.write('\n')
            f.write('tgt:'+' '.join(trans.pred_sents[0]))
            f.write('\n')
            att = trans.attns[0]
            for ii in range(att.size(0)):
                tp = []
                for jj in range(att.size(1)):
                    tp.append(str(round(att[ii][jj], 5)))
                f.write(' '.join(tp)+'\n')
            f.flush()
            f.write('='*10+'\n')
